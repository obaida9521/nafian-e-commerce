...
<div class="max-w-[1280px] mx-auto">
    ...
    <div class="flex items-center gap-1 mb-4">
        <a href="{{ route('admin.customers.index', ['tab' => 'registered'] + request()->only('search')) }}"
           class="h-[34px] px-4 rounded-lg text-sm font-semibold grid place-items-center {{ $tab === 'registered' ? 'bg-wine-700 text-white' : 'bg-white border border-[#EADBC4] text-gray-600 hover:border-wine-700' }}">
            Registered <span class="opacity-70">({{ $registeredCount }})</span>
        </a>
        <a href="{{ route('admin.customers.index', ['tab' => 'guests'] + request()->only('search')) }}"
           class="h-[34px] px-4 rounded-lg text-sm font-semibold grid place-items-center {{ $tab === 'guests' ? 'bg-wine-700 text-white' : 'bg-white border border-[#EADBC4] text-gray-600 hover:border-wine-700' }}">
    ...
    <form method="GET" class="flex gap-2.5 mb-4.5">
        ...
        <input name="search" value="{{ request('search') }}" placeholder="Name, email or phone…" class="h-[38px] w-72 border border-[#EADBC4] rounded-lg px-3 text-sm bg-white outline-none focus:border-wine-700">
        <button class="h-[38px] px-4 rounded-lg bg-wine-700 hover:bg-[#4d141e] text-white text-sm font-semibold">Search</button>
    </form>

    @if($tab === 'guests')
        <div class="bg-white border border-[#EADBC4] rounded-xl overflow-hidden">
            <div class="grid grid-cols-[1.6fr_1.6fr_1.2fr_0.8fr_1fr_1fr] px-5 py-3 text-[11px] tracking-wide uppercase text-gray-400 font-semibold border-b border-[#EFE2CE]">
                <span>Guest</span><span>Email</span><span>Phone</span><span>Orders</span><span>Spent</span><span>First order</span>
            </div>
            @forelse($guests as $g)
                <a href="{{ route('admin.customers.index', ['tab' => 'guests', 'guest' => $g->guest_email] + request()->only('search')) }}" class="grid grid-cols-[1.6fr_1.6fr_1.2fr_0.8fr_1fr_1fr] px-5 py-3 items-center border-b border-[#F6EAD8] text-[13.5px] hover:bg-[#FBEFDD]">
    ...
    @else
        <div class="bg-white border border-[#EADBC4] rounded-xl overflow-hidden">
            <div class="grid grid-cols-[1.6fr_1.6fr_1.2fr_0.8fr_1fr_1fr_0.9fr] px-5 py-3 text-[11px] tracking-wide uppercase text-gray-400 font-semibold border-b border-[#EFE2CE]">
            ...
            @forelse($customers as $c)
                <a href="{{ route('admin.customers.index', ['view' => $c->id] + request()->only('search')) }}" class="grid grid-cols-[1.6fr_1.6fr_1.2fr_0.8fr_1fr_1fr_0.9fr] px-5 py-3 items-center border-b border-[#F6EAD8] text-[13.5px] hover:bg-[#FBEFDD]">
                    <span class="flex items-center gap-2.5">
                        <span class="w-[34px] h-[34px] rounded-full bg-[#F8EAD6] text-wine-700 grid place-items-center font-semibold flex-none">{{ strtoupper(substr($c->name, 0, 1)) }}</span>
...
@if($selected)
    <div x-data="{ open: true }" x-init="$nextTick(() => open = true)">
        <div x-show="open" x-cloak class="fixed inset-0 z-[75]">
            <a href="{{ route('admin.customers.index', request()->only('search')) }}" class="absolute inset-0 bg-[#3c1018]/40 block"></a>
            <div class="absolute top-0 right-0 h-full w-[440px] max-w-[92vw] bg-white shadow-2xl flex flex-col"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-[#EADBC4]">
                    <div class="flex items-center gap-3">
                        <span class="w-11 h-11 rounded-full bg-[#F8EAD6] text-wine-700 grid place-items-center font-semibold text-[17px]">{{ strtoupper(substr($selected->name, 0, 1)) }}</span>
                        <div>
                ...
                <div class="flex-1 overflow-y-auto px-6 py-5.5">
                    ...
                    @forelse($selected->orders as $o)
                        ...
                        <a href="{{ route('admin.orders.show', $o) }}" class="flex items-center justify-between border border-[#EFE2CE] rounded-[9px] px-3.5 py-3 mb-2.5 hover:border-wine-700">
                ...
                <div class="px-6 py-4.5 border-t border-[#EADBC4] flex gap-2.5">
                    <a href="mailto:{{ $selected->email }}" class="flex-1 h-[42px] border border-[#EADBC4] rounded-lg bg-white text-gray-700 text-sm font-semibold grid place-items-center hover:border-wine-700">Email customer</a>
                    <form method="POST" action="{{ route('admin.customers.update', $selected) }}" class="flex-1">
                        ...
                        @if($selected->is_active)
                            <button class="w-full h-[42px] border border-red-300 rounded-lg bg-white text-red-600 text-sm font-semibold hover:bg-red-50">Suspend</button>
                        @else
                            <button class="w-full h-[42px] rounded-lg bg-wine-700 hover:bg-[#4d141e] text-white text-sm font-semibold">Reactivate</button>
                        @endif
                    </form>
                </div>
            </div>
...
@if($selectedGuest)
    <div x-data="{ open: true }" x-init="$nextTick(() => open = true)">
        <div x-show="open" x-cloak class="fixed inset-0 z-[75]">
            <a href="{{ route('admin.customers.index', ['tab' => 'guests'] + request()->only('search')) }}" class="absolute inset-0 bg-[#3c1018]/40 block"></a>
            <div class="absolute top-0 right-0 h-full w-[440px] max-w-[92vw] bg-white shadow-2xl flex flex-col"
                 x-transition:enter="transition ease-out duration-200" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0">
                <div class="flex items-center justify-between px-6 py-5 border-b border-[#EADBC4]">
                    <div class="flex items-center gap-3">
                ...
                <div class="px-6 py-4.5 border-t border-[#EADBC4]">
                    <a href="mailto:{{ $selectedGuest['email'] }}" class="w-full h-[42px] border border-[#EADBC4] rounded-lg bg-white text-gray-700 text-sm font-semibold grid place-items-center hover:border-wine-700">Email guest</a>
                </div>
            </div>
        </div>
    </div>
