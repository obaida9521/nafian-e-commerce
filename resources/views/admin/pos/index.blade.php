...
@section('content')
<div class="max-w-[1320px] mx-auto" x-data="pos(@js($catalog), @js($symbol))">
    <div class="grid lg:grid-cols-[1.7fr_1fr] gap-5 items-start">
        ...
        <div class="bg-white border border-[#EADBC4] rounded-2xl overflow-hidden">
            <div class="p-4 border-b border-[#EFE2CE] flex items-center gap-3">
                <div class="relative flex-1">
                    ...
                    <input x-model="search" type="text" placeholder="Search product, variant or SKU…" autofocus
                        class="w-full h-11 border border-[#EADBC4] rounded-xl pl-10 pr-9 text-sm bg-[#fff2e3] outline-none focus:bg-white focus:border-wine-700">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                    <template x-for="p in filtered" :key="p.variant_id">
                        <button type="button" @click="add(p)" :disabled="p.available < 1"
                            class="group text-left border border-[#EADBC4] rounded-xl overflow-hidden bg-white hover:border-wine-700 hover:shadow-md transition disabled:opacity-45 disabled:cursor-not-allowed disabled:hover:shadow-none disabled:hover:border-[#EADBC4]">
        ...
        {{-- Cart / checkout --}}
        <form method="POST" action="{{ route('admin.pos.store') }}" class="bg-white border border-[#EADBC4] rounded-2xl flex flex-col lg:sticky lg:top-[88px]">
            @csrf
            ...
            <div class="px-5 py-4 border-b border-[#EFE2CE] flex items-center justify-between">
                ...
                <button type="button" @click="cart = []" x-show="cart.length"
                    class="text-[12px] text-red-600 hover:underline">Clear all</button>
            </div>

            <div class="flex-1 max-h-[38vh] overflow-y-auto scrollbar-thin divide-y divide-[#F6EAD8]">
                ...
                <template x-for="item in cart" :key="item.variant_id">
                    <div class="px-4 py-3 flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1.5">
                            <button type="button" @click="dec(item)" class="w-7 h-7 rounded-md border border-[#EADBC4] grid place-items-center text-gray-600 hover:bg-[#fff2e3]">−</button>
                            ...
                            <button type="button" @click="inc(item)" :disabled="item.quantity >= item.available"
                                class="w-7 h-7 rounded-md border border-[#EADBC4] grid place-items-center text-gray-600 hover:bg-[#fff2e3] disabled:opacity-40">+</button>
                        </div>
            ...
            <div class="px-5 py-4 border-t border-[#EFE2CE] space-y-3">
                <div class="grid grid-cols-2 gap-2.5">
                    <input name="customer_name" type="text" placeholder="Customer name (optional)"
                        class="h-9 border border-[#EADBC4] rounded-lg px-3 text-[13px] bg-white outline-none focus:border-wine-700">
                    <input name="customer_phone" type="text" placeholder="Phone (optional)"
                        class="h-9 border border-[#EADBC4] rounded-lg px-3 text-[13px] bg-white outline-none focus:border-wine-700">
                </div>

                <div class="rounded-xl bg-[#fff2e3] border border-[#EFE2CE] p-3.5 space-y-2">
                    ...
                    <div class="flex items-center justify-between text-[13.5px]">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <input name="discount_amount" type="number" min="0" step="0.01" x-model.number="discount"
                                class="w-24 h-8 border border-[#EADBC4] rounded-lg px-2 text-[13px] text-right bg-white outline-none focus:border-wine-700">
                        </div>
                    </div>
                    <div class="flex items-center justify-between text-[16px] font-semibold pt-2 border-t border-[#EADBC4]">
                        <span>Total due</span>
                ...
                <div class="flex flex-wrap gap-1.5">
                    <button type="button" @click="paid = total" class="px-2.5 h-8 rounded-lg border border-[#EADBC4] text-[12px] font-semibold text-gray-700 hover:border-wine-700 hover:bg-[#fff2e3]">Exact</button>
                    <template x-for="amt in quickCash" :key="amt">
                        <button type="button" @click="paid = amt" class="px-2.5 h-8 rounded-lg border border-[#EADBC4] text-[12px] font-semibold text-gray-700 hover:border-wine-700 hover:bg-[#fff2e3]" x-text="money(amt)"></button>
                    </template>
                </div>
                ...
                <div class="flex items-center justify-between text-[13.5px]">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <input name="amount_paid" type="number" min="0" step="0.01" x-model.number="paid"
                            class="w-28 h-9 border border-[#EADBC4] rounded-lg px-2 text-[14px] font-semibold text-right bg-white outline-none focus:border-wine-700">
                    </div>
                </div>
                <div class="flex items-center justify-between text-[13.5px]">
                ...
                <button type="submit" :disabled="cart.length === 0 || paid < total"
                    class="w-full h-12 rounded-xl bg-wine-700 hover:bg-[#4d141e] text-white text-[15px] font-semibold disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition">
