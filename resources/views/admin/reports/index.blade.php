...
<div>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-4.5">
        <div class="flex gap-1.5 bg-[#F5F2F0] p-[3px] rounded-lg">
        ...
        <div class="flex items-center gap-2.5">
            <form method="GET" class="flex items-center gap-2">
                <input type="date" name="from" value="{{ $from->toDateString() }}" class="h-10 border border-[#E9E4E0] rounded-lg px-3 text-sm bg-white outline-none focus:border-wine-700">
                <span class="text-gray-400 text-sm">→</span>
                <input type="date" name="to" value="{{ $to->toDateString() }}" class="h-10 border border-[#E9E4E0] rounded-lg px-3 text-sm bg-white outline-none focus:border-wine-700">
                <button class="inline-flex items-center gap-1.5 h-10 px-4 rounded-lg bg-wine-700 hover:bg-[#2A2220] text-white text-sm font-semibold"><x-ui.icon name="filter" :size="15" />Apply</button>
            </form>
            <a href="{{ route('admin.reports.export', ['from' => $from->toDateString(), 'to' => $to->toDateString()]) }}"
               class="h-10 px-4 border border-[#E9E4E0] rounded-lg bg-white text-gray-700 text-sm font-semibold flex items-center gap-2 hover:border-wine-700">
    ...
    @endphp
    <div class="bg-white border border-[#E9E4E0] rounded-xl p-5.5 mb-4.5">
        ...
        <div class="grid md:grid-cols-2 gap-x-8 gap-y-2 text-[13.5px]">
            <div class="flex justify-between py-2 border-b border-[#F5F2F0]">
        ...
        <div class="mt-4 pt-4 border-t border-[#E9E4E0] flex items-center justify-between">
        ...
        @if($pl['expenses_total'] > 0)
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach($pl['expenses_by_category'] as $cat => $amount)
                    @if($amount > 0)
                        <span class="text-[12px] px-2.5 py-1 rounded-full bg-[#F5F2F0] text-gray-700">
    ...
    <div class="bg-white border border-[#E9E4E0] rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-[#F0ECE9] text-[15px] font-semibold">Top products</div>
        <div class="grid grid-cols-[1fr_100px_120px] px-5 py-3 text-[11px] tracking-wide uppercase text-gray-400 font-semibold border-b border-[#F0ECE9]">
            <span>Product</span><span class="text-right">Units</span><span class="text-right">Revenue</span>
        </div>
        @forelse($topProducts as $p)
            <div class="grid grid-cols-[1fr_100px_120px] px-5 py-3 items-center border-b border-[#F5F2F0] text-[13.5px]">
