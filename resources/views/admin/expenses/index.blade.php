...
<div
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4.5">
        <form method="GET" class="flex items-center gap-2">
            <input type="date" name="month" value="{{ $monthValue }}" onchange="this.form.submit()"
                class="h-10 border border-[#E9E4E0] rounded-lg px-3 text-sm bg-white outline-none focus:border-wine-700">
            <span class="text-[13px] text-gray-500">{{ $monthLabel }}</span>
        </form>
        @adminCan('expenses')
        <button @click="create()" class="h-10 px-4.5 rounded-lg bg-wine-700 hover:bg-[#2A2220] text-white text-sm font-semibold flex items-center gap-2">
            <x-ui.icon name="plus" :size="16" />Add expense
        </button>
        @endadminCan
    </div>
    ...
    <div class="grid lg:grid-cols-[1fr_1.6fr] gap-4.5 mb-4.5">
        ...
        <div class="bg-white border border-[#E9E4E0] rounded-2xl p-5">
            ...
            @forelse($byCategory as $row)
                ...
                <div class="mb-3 last:mb-0">
                    ...
                    <div class="h-1.5 rounded-full bg-[#F5F2F0] overflow-hidden">
    ...
    <div class="bg-white border border-[#E9E4E0] rounded-2xl overflow-hidden">
        <div class="grid grid-cols-[1.6fr_1fr_0.9fr_1fr_110px] px-5 py-3 text-[11px] tracking-wide uppercase text-gray-400 font-semibold border-b border-[#F0ECE9]">
        ...
        @forelse($expenses as $expense)
            ...
            @endphp
            <div class="grid grid-cols-[1.6fr_1fr_0.9fr_1fr_110px] px-5 py-3.5 items-center border-b border-[#F5F2F0] text-[13.5px] hover:bg-[#FBEFDD]">
                <div class="min-w-0">
                ...
                <span class="flex justify-end gap-3.5">
                    @adminCan('expenses')
                    <button @click="edit(@js($payload), @js(route('admin.expenses.update', $expense)))" class="inline-flex items-center gap-1.5 text-wine-700 font-semibold"><x-ui.icon name="edit" :size="14" />Edit</button>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-[75] flex items-center justify-center">
        ...
        <form method="POST" :action="action" class="relative bg-white rounded-[14px] w-[480px] max-w-[92vw] p-6.5 shadow-2xl">
            ...
            <div class="space-y-4 mb-5.5">
                <div><label class="block text-[13px] text-gray-500 mb-1.5">Title</label><input name="title" x-model="form.title" placeholder="Shop rent — June" class="w-full h-[42px] border border-[#E9E4E0] rounded-[7px] px-3.5 text-sm outline-none focus:border-wine-700"></div>
                <div class="grid grid-cols-2 gap-3.5">
                    <div><label class="block text-[13px] text-gray-500 mb-1.5">Category</label>
                        <select name="category" x-model="form.category" class="w-full h-[42px] border border-[#E9E4E0] rounded-[7px] px-3 text-sm bg-white outline-none focus:border-wine-700">
                            @foreach($categories as $category)
                                <option value="{{ $category->value }}">{{ $category->label() }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div><label class="block text-[13px] text-gray-500 mb-1.5">Amount ({{ $symbol }})</label><input name="amount" x-model="form.amount" type="number" step="0.01" min="0" placeholder="0" class="w-full h-[42px] border border-[#E9E4E0] rounded-[7px] px-3.5 text-sm outline-none focus:border-wine-700"></div>
                </div>
                <div><label class="block text-[13px] text-gray-500 mb-1.5">Date</label><input name="spent_on" x-model="form.spent_on" type="date" class="w-full h-[42px] border border-[#E9E4E0] rounded-[7px] px-3.5 text-sm outline-none focus:border-wine-700"></div>
                <div><label class="block text-[13px] text-gray-500 mb-1.5">Notes <span class="text-gray-300">(optional)</span></label><textarea name="notes" x-model="form.notes" rows="2" class="w-full border border-[#E9E4E0] rounded-[7px] px-3.5 py-2.5 text-sm outline-none focus:border-wine-700"></textarea></div>
            </div>
            <div class="flex justify-end gap-2.5">
                <button type="button" @click="open=false" class="inline-flex items-center gap-1.5 h-[42px] px-5 border border-[#E9E4E0] rounded-lg bg-white text-gray-700 text-sm font-semibold"><x-ui.icon name="x" :size="15" />Cancel</button>
                <button class="inline-flex items-center gap-1.5 h-[42px] px-5 rounded-lg bg-wine-700 hover:bg-[#2A2220] text-white text-sm font-semibold"><x-ui.icon name="check" :size="15" /><span x-text="editing ? 'Save changes' : 'Record expense'"></span></button>
            </div>
        </form>
    </div>
</div>
